...
def index(request):
    ...
    if request.query.get("predict_next_token"):
        ...
        next_top_k = [
            [token, log_prob] for log_prob, token in next_top_k
        ]
        ...
        for i, t in enumerate(texts, 1):
            occurred_scores.extend(
                (i, token, log_prob)
                for log_prob, token in model.get_occurred_log_probs(
                    tokenize(t)
                )
            )
...
def to_csv_data_url(data: List[List], header: List[str]) -> str:
    ...
    b64data = base64.b64encode(f.getvalue().encode("utf8")).decode(
        "ascii"
    )
