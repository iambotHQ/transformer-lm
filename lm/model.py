...
class OutputGetters:
    ...
    @staticmethod
    def concat_avg_max_pool(output: torch.Tensor) -> torch.Tensor:
        avg_pool = torch.nn.functional.avg_pool2d(output)
        max_pool = torch.nn.functional.max_pool2d(output)
        return torch.cat((avg_pool, max_pool))
